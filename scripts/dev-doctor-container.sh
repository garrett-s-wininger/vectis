...
TARGETS=${VECTIS_SMOKE_TARGETS:-"scripts/dev-doctor.sh && SKIP_WEB_BUILD=1 make build-container && make proto && make test-quick"}
...
Defaults:
  image   ubuntu:24.04
  targets scripts/dev-doctor.sh && SKIP_WEB_BUILD=1 make build-container && make proto && make test-quick
