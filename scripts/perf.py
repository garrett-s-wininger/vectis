...
This is intentionally small today: it preserves the existing queue benchmark
workflow while adding the artifact and comparison shape needed for a broader
solution-wide performance suite.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    queue = subparsers.add_parser("queue", help="Run queue microbenchmarks.")
    queue.add_argument(
        "--benchtime",
    ...
    queue.add_argument(
        "--bench",
        default=os.getenv("VECTIS_PERF_QUEUE_BENCH", DEFAULT_QUEUE_BENCH),
...
def run_queue_suite(args: argparse.Namespace) -> int:
    command = [
        args.go,
        "test",
        "./internal/queue",
    ...
    run_name = args.run_name or f"{started.strftime('%Y%m%dT%H%M%SZ')}-queue"
