...
Worker execution identity should stay scoped to Vectis-owned action state until worker isolation and a secret broker are in place. If `worker.spire.require_execution_svid` is enabled, exact SVID checks must happen in worker-owned code before an execution is marked started and before action code runs. Do not expose SPIRE Workload API sockets, SVIDs, private keys, or derived execution identity values as ambient shell environment variables for arbitrary job commands. Process-launching actions must use `ExecutionState.CommandEnv()` so worker service env vars are not inherited by job commands.
