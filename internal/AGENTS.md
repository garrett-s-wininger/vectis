...
Worker execution identity should stay scoped to Vectis-owned action state until worker isolation and a secret broker are in place. If `worker.spiffe.enabled` is enabled, exact SVID acquisition must happen in worker-owned code before an execution is marked started and before action code runs, and Workload API fetches must stay bounded by `worker.spiffe.fetch_timeout`. Do not expose SPIFFE Workload API sockets, SVIDs, private keys, or derived execution identity values as ambient environment variables for arbitrary job subprocesses. Process-launching actions must use `ExecutionState.CommandEnv()` so worker service env vars are not inherited by job commands.
...
Builtins live in `internal/action/builtins/`. Currently registered action types are `builtins/script`, `builtins/test`, `builtins/checkout`, `builtins/upload-artifact`, `builtins/sequence`, `builtins/parallel`, `builtins/if`, `builtins/retry`, `builtins/timeout`, `builtins/finally`, `builtins/fallback`, and `builtins/result`; `registry.go` is the builtin resolver, not a user-facing action. Provider-specific actions such as `gerrit/review@v1` live under `extensions/actions/`. Each builtin implements the interface from `internal/action/action.go` and registers its `uses` name so the worker can dispatch to it.
...
| Schedules / trigger polling / recovery | `internal/cron/`, `internal/scmpoller/`, `internal/scmstream/`, `internal/scmtrigger/`, `internal/trigger/`, `internal/reconciler/` · formal model [`../formal/tla/`](../formal/tla/) |
