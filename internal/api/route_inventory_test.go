...
func TestAPIRouteInventory(t *testing.T) {
	...
	want := []struct {
		...
		{"POST /api/v1/source-repositories/{id}/definitions/import", string(authz.ActionJobWrite)},
		...
		{"GET /api/v1/jobs/{id}/source", string(authz.ActionJobRead)},
		{"GET /api/v1/jobs/{id}/source/definition", string(authz.ActionJobRead)},
		{"POST /api/v1/jobs/source/{id}", string(authz.ActionJobWrite)},
		{"PUT /api/v1/jobs/source/{id}", string(authz.ActionJobWrite)},
