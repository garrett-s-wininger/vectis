...
func TestSecretsKnoxConfig(t *testing.T) {
	viper.Reset()
	t.Cleanup(viper.Reset)

	if got := SecretsKnoxURL(); got != "" {
		t.Fatalf("SecretsKnoxURL default = %q, want empty", got)
	}

	if got := SecretsKnoxAuthTokenFile(); got != "" {
		t.Fatalf("SecretsKnoxAuthTokenFile default = %q, want empty", got)
	}

	if got := SecretsKnoxAuthToken(); got != "" {
		t.Fatalf("SecretsKnoxAuthToken default = %q, want empty", got)
	}

	if got := SecretsKnoxInsecureSkipVerify(); got {
		t.Fatal("SecretsKnoxInsecureSkipVerify default = true, want false")
	}

	viper.Set("knox_url", "https://knox.local")
	viper.Set("knox_auth_token_file", "/run/knox/token")
	viper.Set("knox_auth_token", "0m-flat")
	viper.Set("knox_insecure_skip_verify", true)

	if got := SecretsKnoxURL(); got != "https://knox.local" {
		t.Fatalf("SecretsKnoxURL flat override = %q", got)
	}

	if got := SecretsKnoxAuthTokenFile(); got != "/run/knox/token" {
		t.Fatalf("SecretsKnoxAuthTokenFile flat override = %q", got)
	}

	if got := SecretsKnoxAuthToken(); got != "0m-flat" {
		t.Fatalf("SecretsKnoxAuthToken flat override = %q", got)
	}

	if got := SecretsKnoxInsecureSkipVerify(); !got {
		t.Fatal("SecretsKnoxInsecureSkipVerify flat override = false, want true")
	}

	viper.Reset()
	t.Cleanup(viper.Reset)
	viper.Set("secrets.knox.url", "https://knox.service")
	viper.Set("secrets.knox.auth_token_file", "/etc/knox/token")
	viper.Set("secrets.knox.auth_token", "0m-namespaced")
	viper.Set("secrets.knox.insecure_skip_verify", true)

	if got := SecretsKnoxURL(); got != "https://knox.service" {
		t.Fatalf("SecretsKnoxURL namespaced override = %q", got)
	}

	if got := SecretsKnoxAuthTokenFile(); got != "/etc/knox/token" {
		t.Fatalf("SecretsKnoxAuthTokenFile namespaced override = %q", got)
	}

	if got := SecretsKnoxAuthToken(); got != "0m-namespaced" {
		t.Fatalf("SecretsKnoxAuthToken namespaced override = %q", got)
	}

	if got := SecretsKnoxInsecureSkipVerify(); !got {
		t.Fatal("SecretsKnoxInsecureSkipVerify namespaced override = false, want true")
	}
}
