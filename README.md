...
Portable build targets are moving to Mage. Use `mage -l` to list the targets after running the doctor install and loading the repo-local toolchain environment. The current Makefile delegates `doctor`, `build`, `proto`, `build-container`, `format`, `test`, `test-e2e`, `test-e2e-deploy-linux`, `test-integration`, `test-lima`, `test-postgres-integration`, `test-quick`, and `test-race` to Mage so those targets have one implementation.
