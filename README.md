...
Portable build targets are moving to Mage. Use `mage -l` to list the targets after running the doctor install and loading the repo-local toolchain environment. The current Makefile delegates common dev targets (`doctor`, `build`, `proto`, `build-container`, `format`, `docs-assets`, `clean`), deploy artifact targets (`deploy-artifacts-test`, `deploy-artifacts-render`), operational helpers (`podman-grafana-configmaps`, `perf`, `perf-compare`), release readiness (`release-readiness-report`), quality targets (`lint-api-routes`, `lint`, `vulncheck`, `fuzz-api-auth`), and the simple Go test targets (`test`, `test-quick`, `test-race`, `test-integration`, `test-postgres-integration`, `test-e2e`, `test-e2e-deploy-linux`, `test-lima`, `test-fault`, `test-property`) to Mage so those targets have one implementation.
