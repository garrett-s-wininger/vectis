...
This page defines the compatibility contract for Vectis clients, operators, and alternate component implementations. It complements [API_REFERENCE.md](API_REFERENCE.md), [MIGRATIONS.md](MIGRATIONS.md), and [RELEASES.md](RELEASES.md).
...
- Error responses use the envelopes documented in [API_REFERENCE.md](API_REFERENCE.md). New `code` values may be added for more specific failures; existing code meanings should remain stable.
...
Only routes listed in [API_REFERENCE.md](API_REFERENCE.md) and [IDEMPOTENCY_AND_RETRIES.md](IDEMPOTENCY_AND_RETRIES.md) accept `Idempotency-Key`. Adding idempotency support to another mutating route is compatible. Removing idempotency support, changing key scope, or changing replay behavior is breaking.
...
- Service env prefixes are part of the contract for each binary; update [CONFIGURATION.md](CONFIGURATION.md) and [cmd/AGENTS.md](../cmd/AGENTS.md) together when they change.

## Schema And Releases

Database compatibility follows [MIGRATIONS.md](MIGRATIONS.md) and [ADR 0004](adr/0004-migration-compatibility-and-rollback.md). Release notes must say whether old binaries can run against the new schema, whether new binaries can start before migration, and whether mixed binary versions are supported.

Default deployment guidance remains: run one Vectis release version across long-running services unless [RELEASES.md](RELEASES.md) or release notes explicitly allow version skew.
