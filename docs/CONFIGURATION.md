...
**`make deploy-podman`** ([`deploy/podman/kube-spec.yaml`](../deploy/podman/kube-spec.yaml)) runs an **init container** that generates a pod-local CA and a **shared** server certificate (SAN **localhost** and **127.0.0.1**) into a volume mounted at **`/run/vectis/grpc-tls`** on every Vectis container. ConfigMap **`vectis-grpc-tls-env`** sets **`VECTIS_GRPC_TLS_INSECURE=false`**, PEM paths, and **`VECTIS_GRPC_TLS_SERVER_NAME=localhost`** so clients accept registry addresses published as **127.0.0.1**. The init image runs **`apk add openssl`** (needs **network** on a cold cache).
