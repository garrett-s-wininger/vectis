...
A future `vectis-cli doctor` could automate common post-restore checks: database reachability, schema version, API readiness, queue health, log health, reconciler age, and presence of required TLS/secret files. Until then, the smoke test above is the operator checklist.
