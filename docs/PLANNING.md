...
**Shipped reference:** [`deploy/podman/kube-spec.yaml`](../deploy/podman/kube-spec.yaml) and **`vectis-cli deploy podman up`** (`podman play kube`) — see [README.md](../README.md). The same spec includes **Prometheus** and **Grafana** alongside Vectis services for a **demo monitoring bundle** (see §10).
