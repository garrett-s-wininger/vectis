...
3. If there is no dispatch event, follow [DISPATCH_VISIBILITY.md#runbook-queued-with-no-dispatch](DISPATCH_VISIBILITY.md#runbook-queued-with-no-dispatch).
4. If dispatch failed, follow [DISPATCH_VISIBILITY.md#runbook-dispatch-failure](DISPATCH_VISIBILITY.md#runbook-dispatch-failure).
...
2. Confirm exactly one active reconciler for the current scale posture; see [SCALING_AND_RESTARTS.md](SCALING_AND_RESTARTS.md).
...
6. For restore-specific order and partial-restore outcomes, use [BACKUP_RESTORE.md](BACKUP_RESTORE.md).
