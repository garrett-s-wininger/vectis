...
registry, Gerrit stream bridge, SCM poller, secrets, SPIFFE authority, worker,
and worker-core packages. The common package also ships the timer-driven
retention cleanup workflow that uses `vectis-cli`; operators enable it only
after supplying backup evidence, expected topology, and API credentials.
