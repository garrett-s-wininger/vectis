...
func main() {
	...
	fmt.Fprintf(os.Stdout, "Gerrit smoke succeeded: project=%s change=%s revision=%s fetch_ref=%s poll_discovered=%t scm_event=%t checkout=%t review=%t wrong_password_denied=%t job=%t job_review=%t poller_backstop=%t\n", result.Project, result.Change, result.Revision, result.FetchRef, result.PollDiscovered, result.SCMEventEmitted, result.CheckoutVerified, result.ReviewPosted, result.WrongPasswordDenied, result.JobExecuted, result.JobReviewPosted, result.PollerBackstop)
}
