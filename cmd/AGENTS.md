...
| `vectis-orchestrator` | `VECTIS_ORCHESTRATOR` | `[orchestrator]`; owns in-memory run state shards, exposes the task choreography gRPC service used by workers, and uses `config.GRPCServerOptionsForRole(config.ServiceIdentityRoleOrchestrator)` |
