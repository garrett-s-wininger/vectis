...
| `vectis-cell-ingress` | `VECTIS_CELL_INGRESS` | `[cell_ingress]`; private HTTP `POST /cell/v1/executions`, `--allowed-host` / `VECTIS_CELL_INGRESS_ALLOWED_HOSTS` configure accepted Host headers, local execution repair, metrics host/port, plus queue discovery/pinned queue settings |
