...
func init() {
	...
	deployLinuxLimaCmd.PersistentFlags().StringVar(&linuxLimaInstance, "instance", linuxLimaInstance, "Lima instance name for Linux deploy smoke tests")
	deployLinuxLimaCmd.PersistentFlags().StringVar(&linuxLimaTemplate, "template", linuxLimaTemplate, "Lima template used when creating the Linux deploy smoke instance")
	deployLinuxLimaVerifyCmd.Flags().StringVar(&linuxLimaArtifactDir, "artifacts", linuxLimaArtifactDir, "Optional directory where rendered Linux artifacts are written before copying into Lima; defaults to a temporary directory")
	deployLinuxLimaVerifyCmd.Flags().BoolVar(&linuxLimaKeepArtifacts, "keep-artifacts", false, "Leave rendered artifacts and installed smoke files in place after verification")
	deployLinuxLimaCmd.AddCommand(deployLinuxLimaVerifyCmd)
	deployLinuxLimaCmd.AddCommand(deployLinuxLimaCleanCmd)
	deployLinuxLimaCmd.AddCommand(deployLinuxLimaDownCmd)
	deployLinuxLimaCmd.AddCommand(deployLinuxLimaDeleteCmd)
	deployLinuxCmd.AddCommand(deployLinuxRenderCmd)
	deployLinuxCmd.AddCommand(deployLinuxLimaCmd)
