...
- **Control:** You run the stack; there is no required hosted control plane. Data and execution stay on infrastructure you choose. Trust boundaries for the open HTTP surface are in [SECURITY.md](../general/security.md).
- **Operability:** Predictable failure behavior and calm upgrade paths; see [FAILURE_DOMAINS.md](../general/failure-domains.md). Prefer explicit steps (migrations, Podman/Kube-style deploy) over magic.
...
- **Persistence:** **SQLite** by default; **PostgreSQL** via `pgx` with the same embedded migrations (§2.5, [CONFIGURATION.md](../operator/configuration.md)). Supporting **every** SQL vendor or dialect is **not** a goal. Multi-instance and HA are **operational** concerns on the Postgres path.
...
**Also:** [CONFIGURATION.md](../operator/configuration.md) (env and flags), [FAILURE_DOMAINS.md](../general/failure-domains.md) (outages), [SECURITY.md](../general/security.md) (trust model for the open HTTP API), [GLOSSARY.md](../general/glossary.md), [adr/README.md](adr/README.md).
...
See [FAILURE_DOMAINS.md](../general/failure-domains.md). **Security:** [SECURITY.md](../general/security.md).
...
- **Largely met:** standalone docs exist ([ARCHITECTURE.md](architecture.md), [CONFIGURATION.md](../operator/configuration.md), [FAILURE_DOMAINS.md](../general/failure-domains.md), [GLOSSARY.md](../general/glossary.md), [SECURITY.md](../general/security.md), [FEDERATION.md](federation.md) as deferred target, [adr/README.md](adr/README.md)). **Ongoing:** keep **§2–§3** aligned with the repo on each release; **§4+** remains target spec.
...
- **REST errors:** JSON with `code`, `message`, and optional `details` as documented in [API_REFERENCE.md](../user/api-reference.md).
...
**System / component logs:** processes still log primarily to **stderr** (structured JSON optional on the API—[CONFIGURATION.md](../operator/configuration.md)). **Central aggregation** (Loki, cloud log sinks, OTEL log export) is a **target** ops concern. **Prometheus metrics** for core services are **shipped** (§10).
...
**Shipped defaults** are embedded in `internal/config/defaults.toml` (API, registry, queue, log gRPC/SSE ports, discovery timings, database driver/DSN template). Runtime configuration is primarily **environment variables** per service — see [CONFIGURATION.md](../operator/configuration.md). The `config.toml` fragment below is **illustrative** only (Viper/TOML file loading is not the primary path today). Keys for heartbeat, unified triggers, multi-queue remain **target-only**.
...
| API security | Auth, RBAC, rate limits, audit logging | **Shipped** — see [SECURITY.md](../general/security.md) |
...
| Observability | Prometheus **metrics** on API, queue, worker, log; **Grafana + Prometheus** in Podman kube demo; traces / centralized logs / full OTEL stack | **Partial** — [CONFIGURATION.md](../operator/configuration.md), §10, `deploy/grafana/dashboards/` |
