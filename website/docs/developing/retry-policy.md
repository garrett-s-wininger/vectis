...
| Task continuation enqueue | Worker direct fan-out after orchestrator completion | One enqueue attempt for each activated child returned by the orchestrator after the worker mirrors child activation and queued run state to SQL | No direct retry; persisted continuations are left for the reconciler queue policy | No | Worker traces include `task.dispatch.persisted` and `task.dispatch.direct`; queue backlog and worker outcome metrics show pressure | If SQL continuation state was persisted, the run remains queued and the reconciler redispatches all pending child executions for that run. If persistence fails first, the worker reports the handoff failure. |
...
| Worker orchestrator claim/lease/finalize recovery | Worker | Claim can hydrate missing hot state once for non-root deliveries; lease renew retries on the next interval and can hydrate/reclaim missing hot state; completion retries 4 attempts and can hydrate/reclaim before retrying with a fresh claim token | 150ms exponential capped at 2s for completion | No | Domain logs/spans and `vectis_worker_orchestrator_recoveries_total` labelled by `stage` | Run can be left for queue delivery, catalog visibility, or operator/reconciler recovery depending on where exhaustion occurs. |
...
| Durable log stream send | Worker task delivery execution | Retries until close flush deadline; pending spool replay quarantines semantically unrecoverable files such as stale or missing run IDs | 150ms exponential capped at 2s | No | Domain logs, no common counter today | Run fails if logs cannot flush before the deadline. |
...
| Log-forwarder delivery | Log-forwarder | Retries through persisted local spool behavior | See log-forwarder configuration and logs | No | Domain logs, no common counter today | Batch remains spooled or is quarantined when corrupt or semantically unrecoverable. |
