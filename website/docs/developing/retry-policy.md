...
| Task continuation enqueue | Worker task fan-out dispatcher | One enqueue attempt per drain pass; failed intents remain pending after their retry cutoff | Retry cutoff is stored on the task dispatch intent | No | Per-run dispatch events with source `task_dispatch`; worker emits `vectis_task_dispatch_drains_total` and `vectis_task_dispatch_intents_total` | The task intent remains pending for a later worker drain pass. |
