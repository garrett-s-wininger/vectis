...
- **Client semantics:** Success of the HTTP call means the run is **recorded**, not guaranteed to be **on the queue** yet. Operators and integrators should run the **reconciler** and monitor for stuck **queued** runs ([0002](./0002-standalone-reconciler-process.md), [FAILURE_DOMAINS.md](../../concepts/failure-domains.md)).
