...
Plan: [SCALING_AND_RESTARTS.md](../../operating/deployment/scaling-and-restarts.md)

First pass: [SCALING_AND_RESTARTS.md](../../operating/deployment/scaling-and-restarts.md) documents recommended replica counts, current multi-replica safety, rolling-restart expectations, probe guidance, and scale-out boundaries. **Open:** human review for long-term API rate-limit behavior, queue/log HA stance, cron leader election or partitioning, reconciler duplicate-handoff bounds, pool-aware worker scale-out, and rolling-restart tests.

### 6. Retention And Storage Pressure

**Status:** defer

First pass: [RETENTION.md](../../operating/reliability/retention.md) documents `vectis-cli retention cleanup`, default windows, active-run protections, local run-log pruning, audit events, and SQL pressure metrics. **Open:** Human review for production retention windows, scheduled cleanup authority, backup/restore interaction, queue persistence, log-forwarder spools, and deploy-specific disk pressure behavior.
...
- [SCALING_AND_RESTARTS.md](../../operating/deployment/scaling-and-restarts.md)
- [RETENTION.md](../../operating/reliability/retention.md)
- [CLI_OPERATIONAL_COVERAGE.md](../../operating/reference/cli-operational-coverage.md)
- [BACKUP_RESTORE.md](../../operating/reliability/backup-restore.md)
- [FAILURE_DOMAINS.md](../../concepts/failure-domains.md)
