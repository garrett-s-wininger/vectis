...
| Restore drill is mostly manual | A focused local restore drill test now backs up and restores SQLite plus queue, log, artifact, log-forwarder spool, and worker pending log-spool paths; reopens restored state; reruns migrations; emits restored inventory, manifest, and storage reports; and requires the combined proof through `backup verify --storage-report`. A local e2e restore drill now starts the stack on isolated ports, runs the canonical smoke job, copies the data home as backup media, verifies restored backup evidence, restarts from the restored data home, confirms the pre-restore run/logs/artifacts, and runs a post-restore smoke job. Remaining work is a reference deployment drill with external DB/platform backup media. | Add an e2e restore-drill lane for the reference deployment with service startup, health, secret/TLS checks, and a triggered smoke run from platform-restored media. |
| File-store integrity checks need drill automation | `vectis-cli storage verify` now checks queue snapshot/WAL files, durable run logs, artifact blob SHA-256 path digests, log-forwarder CRC spools, and worker pending log spools. `backup verify --storage-report` can require matching fresh `ok` reports for every storage-backed local-state path, retention cleanup can require the same proof before deletion, and the local CLI plus e2e restore drills exercise this proof path after restore. Remaining work is production-readiness gate integration and proof artifact retention. | Add reference-deployment restore-drill automation that stores the combined storage-report plus backup-manifest proof artifact. |
...
| Retention backup gate MVP | `vectis-cli retention cleanup` can verify a backup manifest, optional expected topology, optional max manifest age, and optional storage integrity reports before dry-run or apply; output includes backup proof fields when supplied. | CLI tests cover missing manifest flag combinations, failed manifest verification, stale manifest rejection, fresh expected-topology success, fresh storage reports, and stale storage report rejection. |
