...
| Retention compliance controls are only partially linked | `vectis-cli retention cleanup` can now require backup manifest freshness and audit export evidence, and run-scoped `vectis-cli retention holds` can protect selected terminal runs plus related run state from cleanup. Remaining work is explicit operator waivers, audit-range holds, and policy checks that decide when backup-manifest, audit-export, or hold review is mandatory. | Add waiver evidence fields, audit-range holds, and policy checks for mandatory backup-manifest, audit-export, and hold-review enforcement. |
...
| Audit retention can still erase evidence unless operators opt into the gate | Retention deletes old `audit_log` rows by policy. `vectis-cli audit export` can now produce retained evidence, and `retention cleanup --audit-export` can verify it before deleting audit rows. Remaining work is making this mandatory by policy where required and adding audit-range holds. | Add audit-range holds, policy-required audit export checks, and waiver evidence for approved cleanup without export evidence. |
...
| Audit export MVP | Admin API and CLI list/export for `audit_log` landed with cursor pagination, bounded filtering, JSON evidence output, event hashing, truncation detection, and optional retention cleanup verification through `--audit-export`. | Add optional CSV output, signed export manifests, mandatory policy hooks, and audit-range hold support. |
