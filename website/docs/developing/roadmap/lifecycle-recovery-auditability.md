...
| File-store integrity checks need drill automation | `vectis-cli storage verify` now checks queue snapshot/WAL files, durable run logs, artifact blob SHA-256 path digests, log-forwarder CRC spools, and worker pending log spools. `backup verify --storage-report` can require matching fresh `ok` reports for every storage-backed local-state path, and retention cleanup can require the same proof before deletion. Remaining work is executing this automatically in restore drills and production-readiness gates. | Add restore-drill automation that emits storage reports, attaches them to backup manifest verification, and stores the combined proof artifact. |
