...
| Startup preflight contract is not tabular | Dependency checks are in code and docs, but there is no single matrix of "what must be true before this binary accepts work." | Add a lifecycle contract table to the operating reference and keep it tested against command wiring. |
...
| Gap | Why it matters | Likely fix |
| --- | --- | --- |
| No backup inventory command | Docs list the state surfaces, but operators must manually assemble DBs, queue WALs, log/artifact stores, spools, secrets, TLS, config, and dashboards. | Add `vectis-cli backup inventory --format json` that emits version, schema, redacted DSNs, storage paths, instance IDs, and local path visibility. |
| No backup set manifest or verifier | Same-window backup consistency is a documented goal, but there is no machine-readable manifest to compare during restore. | Emit/validate a backup manifest with timestamps, Vectis version, schema version, queue/log/artifact shard IDs, and file-store roots. |
...
| Retention and backup freshness are not linked | Retention can remove audit rows, terminal run data, log files, and artifact blobs. Docs warn operators, but the command does not know whether a fresh backup exists. | Add optional backup-manifest checks or explicit waiver fields to retention apply flows. |
...
| Audit retention can erase evidence without export workflow | Retention deletes old `audit_log` rows by policy. There is no first-party export/hold mechanism before cleanup. | Add audit export, retention dry-run evidence, and optional hold/manifest checks before deleting audit rows. |

## First Work Packages

| Package | Scope | Suggested tests |
| --- | --- | --- |
| Lifecycle contract matrix | Add a docs/reference table for each binary's startup checks, readiness, drain behavior, state closed on exit, and hard-stop fallback. | Static test or golden check that every `cmd/*` daemon appears in the matrix. |
...
| Backup inventory MVP | Add `vectis-cli backup inventory --format json` for schema, redacted DB role DSNs, queue/log/artifact/spool paths, secrets/TLS path visibility, and service instance IDs. | CLI tests for redaction, missing paths, and split global/cell DB config. |
