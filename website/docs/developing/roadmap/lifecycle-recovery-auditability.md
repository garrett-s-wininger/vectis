...
| Audit query/export is only an MVP | Admin-only `GET /api/v1/audit/events` and `vectis-cli audit list --format json` now cover newest-first audit review and bounded JSON export with filters for time, type, actor, target, and correlation ID. | Add cursor pagination, CSV output if operators need it, signed export manifests, and retention checks that require audit-range export evidence before deleting old rows. |
...
| Audit export MVP | Admin API and CLI list/export for `audit_log` landed with bounded filtering and JSON evidence output. | Add cursor pagination, optional CSV output, signed export manifests, and retention policy hooks for audit-range export evidence. |
