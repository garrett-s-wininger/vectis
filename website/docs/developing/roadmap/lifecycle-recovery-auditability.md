...
| Retention and backup freshness are only partially linked | `vectis-cli retention cleanup` can now require `--backup-manifest`, `--backup-expect`, and `--backup-max-age` before cleanup, so automation can fail stale or incomplete backup evidence before deletion. Remaining work is explicit operator waivers and audit/export holds for environments that intentionally apply without a current manifest. | Add waiver evidence fields, audit export/hold workflow, and policy checks that decide when backup-manifest enforcement is mandatory. |
