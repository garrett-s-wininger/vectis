...
| Gap | Why it matters | Likely fix |
| --- | --- | --- |
| Backup inventory needs deployment breadth | `vectis-cli backup inventory --format json` now emits version, schema visibility, redacted DB role DSNs, local state paths, instance IDs, and secret/TLS path readability for the current host. Remaining work is cross-host aggregation, manifest comparison, and deploy-specific config/dashboard inventory. | Add backup manifest capture/verify and reference-deploy aggregation that can prove every expected host and shard contributed inventory. |
