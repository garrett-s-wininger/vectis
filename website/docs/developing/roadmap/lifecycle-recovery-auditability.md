...
| File-store integrity checks need backup-manifest integration | `vectis-cli storage verify` now checks queue snapshot/WAL files, durable run logs, artifact blob SHA-256 path digests, log-forwarder CRC spools, and worker pending log spools. Remaining work is tying those reports into backup manifests and restore-drill automation. | Add optional manifest references to storage verification reports and require fresh reports in destructive or production-readiness gates where appropriate. |
