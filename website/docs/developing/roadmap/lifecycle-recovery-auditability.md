...
| Retention compliance controls are only partially linked | `vectis-cli retention cleanup` can now require backup manifest freshness and audit export evidence, and `vectis-cli retention holds` can protect selected terminal runs plus related run state or `audit_log` time ranges from cleanup. Remaining work is explicit operator waivers and policy checks that decide when backup-manifest, audit-export, or hold review is mandatory. | Add waiver evidence fields and policy checks for mandatory backup-manifest, audit-export, and hold-review enforcement. |
...
| Audit retention can still erase evidence unless operators opt into the gate | Retention deletes old unheld `audit_log` rows by policy. `vectis-cli audit export` can now produce retained evidence, `retention cleanup --audit-export` can verify it before deleting audit rows, and audit-range holds can preserve selected review windows. Remaining work is making export/hold review mandatory by policy where required. | Add policy-required audit export checks and waiver evidence for approved cleanup without export or hold-review evidence. |
