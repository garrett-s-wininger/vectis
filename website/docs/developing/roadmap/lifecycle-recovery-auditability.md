...
| Audit query/export is only an MVP | Admin-only `GET /api/v1/audit/events`, `vectis-cli audit list --format json`, and `vectis-cli audit export` now cover newest-first audit review, bounded JSON export, retained export evidence, and an optional retention gate. | Add cursor pagination, CSV output if operators need it, signed export manifests, and policy checks that require audit-range export evidence before deleting old rows. |
...
| Audit export MVP | Admin API and CLI list/export for `audit_log` landed with bounded filtering, JSON evidence output, event hashing, truncation detection, and optional retention cleanup verification through `--audit-export`. | Add cursor pagination, optional CSV output, signed export manifests, mandatory policy hooks, and audit-range hold support. |
