...
| Reference deploy | Render, start, stop, and inspect the Podman reference deployment, including `--profile simple` and `--profile ha`; render the initial Kubernetes reference manifest. | `vectis-cli deploy podman init`, `render`, `up`, `status`, `down`; `vectis-cli deploy kubernetes render` |
