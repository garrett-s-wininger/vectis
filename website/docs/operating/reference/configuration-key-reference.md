...
| `worker.execution.checkout_cache_lease_ttl` | `1h` | Maximum age for a worker-core checkout cache generation lease before cleanup treats it as stale. Set above the longest expected cache-backed checkout clone duration. |
