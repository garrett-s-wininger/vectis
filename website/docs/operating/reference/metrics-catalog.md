...
| `vectis_source_ref_hydrations_total` | Counter | `source_kind`, `checkout_mode`, `outcome`, `reason`, `tier`, `cache` | Request-time managed ref hydration attempts after a source ref misses locally. `tier` identifies the remote tier that satisfied the request, such as `origin` or `fallback-2`; `coalesced` means another API replica's in-flight lease/fetch made the ref available locally; `cache` indicates whether a recent hydration steered the retry order. | Detect mirror lag, fallback-tier dependence, duplicate-fetch suppression, and repeated failures for fresh refs in high-churn repositories. |
| `vectis_source_ref_hydration_duration_seconds` | Histogram | `source_kind`, `checkout_mode`, `outcome`, `reason`, `tier`, `cache` | Wall time spent hydrating a missing managed source ref, including short waits behind another API replica's hydration lease. | Track request-path latency from targeted fetches, lease coalescing, and origin versus fallback tier performance. |
| `vectis_source_repository_object_store_pressure` | Gauge | `repository_id`, `source_kind`, `checkout_mode`, `pressure` | Latest checkout object-store pressure rank recorded during source status/sync probes: `0` ok, `1` warning, `2` critical. | Alert on repositories drifting toward expensive Git maintenance or poor fetch/resolve latency. |
...
| `vectis_worker_checkout_cache_warm_remotes_total` | Counter | `outcome` | Remote-level warm results. `outcome` is `changed`, `unchanged`, `warmed`, or `failed`; `warmed` is retained for older worker-core responses that do not split changed versus unchanged refreshes. | Compare failed versus successful remotes, and changed versus unchanged remotes, to spot mirror or upstream access problems and tune warm cadence. |
...
| `vectis_checkout_cache_clones_total` | Counter | `mode`, `reason` | Cache-backed workspace clone attempts by local object transfer mode. `mode` is `hardlink`, `copy`, or `borrowed`; `reason` is `ok`, `probe`, or `retry`. | Detect workers whose cache and workspace filesystems force copy-mode or borrowed-object checkouts instead of hardlink-mode checkouts. |
| `vectis_checkout_cache_demand_hydrations_total` | Counter | `outcome` | Worker-core checkout cache demand hydrations for auxiliary refs requested by checkout actions. `outcome` is `success` or `failed`. | Track how often dynamic refs such as pull-request refs, Gerrit refs, or notes require just-in-time mirror hydration, and alert on failures. |
| `vectis_checkout_cache_generation_evictions_total` | Counter | `reason` | Worker-core checkout cache mirror generations evicted by cleanup. `reason` is `retention`, `budget`, or `corrupt`. | Separate ordinary retention cleanup from byte-budget pressure and corrupt-generation self-heal cleanup. |
| `vectis_checkout_cache_self_heals_total` | Counter | `operation`, `outcome` | Worker-core checkout cache self-heal attempts after corrupt or missing cached objects. `operation` is `checkout` or `fetch_refspec`; `outcome` is `success` or `failed`. | Alert on repeated failed self-heals and correlate successful self-heals with upstream mirror churn or local filesystem issues. |
