...
| `vectis_checkout_cache_clones_total` | Counter | `mode`, `reason` | Cache-backed workspace clone attempts by local object transfer mode. `mode` is `hardlink` or `copy`; `reason` is `ok`, `probe`, or `retry`. | Detect workers whose cache and workspace filesystems force copy-mode checkouts instead of hardlink-mode checkouts. |
