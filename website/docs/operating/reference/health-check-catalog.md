...
| `source.checkout.filesystem` | warning | Local `VECTIS_SOURCE_CHECKOUT_ROOT`, `VECTIS_API_SERVER_SOURCE_CHECKOUT_ROOT`, or default data path | Managed source checkout root is absolute, and the root or nearest existing parent is inspectable with at least 1 GiB free. If the root already exists, it must be writable. | Free disk space, fix checkout root ownership, or move managed source checkouts to a larger writable volume. |
...
| `secrets.encryptedfs.files` | warning | Local `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_ROOT` and `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_KEY_FILE` paths | Encryptedfs is disabled, or both paths are configured, the root exists and is writable, the key file is regular and private, and the key parses as a valid encryptedfs key. | Check encryptedfs secret root/key mounts, ownership, permissions, and backup placement. |
...
| `artifact.storage.filesystem` | warning | Local `VECTIS_ARTIFACT_STORAGE_DIR` or default data path | Durable local artifact storage directory, or nearest existing parent, is inspectable and has at least 1 GiB free. Skipped when `VECTIS_ARTIFACT_STORAGE_BACKEND=s3`. | Free disk space or move local artifact storage to a larger writable volume. For S3-compatible storage, monitor the object store separately. |
...
When `source.checkout.filesystem` warns, `evidence` includes only local
filesystem fields for the managed checkout root, for example `path=/var/lib/vectis/source-checkouts stat_path=/var/lib/vectis free_bytes=...`.
It does not list repository URLs, credential refs, branch names, or checkout contents.
...
When `service.identity.config` warns, `evidence` includes allowlist and identity
counts plus mTLS prerequisite booleans, for example `allowlists=2 identities=3
