...
| `db.connection.pool` | warning | `GET /api/v1/db/pool-stats` | Pool is not fully in use with recorded waits. | [Database Pool Pressure](../reliability/repair-runbooks.md#database-pool-pressure). |
| `queue.backlog.ratio` | warning | `GET /api/v1/queue/backlog` | Queued run count is within the built-in threshold. | [Queued Runs Or Backlog](../reliability/repair-runbooks.md#queued-runs-or-backlog). |
