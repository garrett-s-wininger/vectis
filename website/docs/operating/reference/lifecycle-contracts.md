...
| `vectis-ui` | Builds the UI HTTP handler from embedded files, an override directory, or a Vite dev assets proxy; validates the API backend URL and opens the listener. | `GET /health/live`; UI has no workflow readiness dependency beyond API requests proxying to the configured backend. | Bounded HTTP shutdown with the shared helper. | No workflow state. Embedded or configured static assets remain outside request state. | UI HTTP shutdown timeout, then supervisor kill. |
...
| `vectis-scm-poller` | Validates client gRPC TLS and cell-ingress HTTP mTLS, waits for the global DB schema, initializes configured SCM providers, and connects to queue. | Supervisor process state plus DB, queue, and provider reachability; no HTTP readiness endpoint. | Stops the polling loop on context cancellation. | Closes DB and queue dial resources. SCM trigger claims, provider event keys, and dispatched runs remain in SQL for retry or dedupe after restart. | Supervisor grace period; stale trigger claims expire after the SCM poller claim TTL. |
| `vectis-scm-gerrit-stream` | Validates client gRPC TLS and cell-ingress HTTP mTLS, waits for the global DB schema, configures Gerrit stream transport, and connects to queue. | Supervisor process state plus DB, queue, and Gerrit stream reachability; no HTTP readiness endpoint. | Stops consuming stream events on context cancellation and exits the active transport loop. | Closes DB and queue dial resources. Recorded Gerrit event keys and dispatched runs remain in SQL for dedupe and repair after restart. | Supervisor grace period; upstream transport reconnect policy controls transient stream failures before process exit. |
