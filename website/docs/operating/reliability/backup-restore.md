...
1. Stop API, cell ingress, cron, SCM trigger producers, reconciler, catalog, workers, worker-core, orchestrator, queue, log, artifact, secrets, spiffe, and log-forwarder processes so no restored state is modified while files are being replaced.
...
7. Start registry, queue, orchestrator, log, artifact, spiffe, and secrets first; then cell ingress, API, worker-core, workers, cron, SCM trigger producers, reconciler, catalog, and log-forwarder.
8. Run the restore smoke test below.

Do not start cron, SCM trigger producers, or workers before the database has been restored and migrations have been checked. They can enqueue, record trigger events, or execute work against an incomplete view of the world.
...
2. Stop API, cell ingress, cron, SCM trigger producers, reconciler, catalog, workers, worker-core, orchestrator, queue, log, artifact, secrets, spiffe, and log-forwarder containers/processes.
