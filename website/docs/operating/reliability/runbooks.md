...
| Worker | Worker-core and worker shell Unix socket presence and private permissions when local paths are visible. |
...
| Source sync | `vectis_source_repository_syncs_total` and `vectis_source_repository_sync_duration_seconds` by `trigger`, `source_kind`, `checkout_mode`, `outcome`, and `reason`. | Source repository sync failures should match known repository, credential, or network changes and should not remain failed before scheduled triggers depend on them. |
