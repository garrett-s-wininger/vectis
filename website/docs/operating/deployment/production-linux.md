...
worker, cron, Gerrit stream bridge, SCM poller, catalog, reconciler, secrets,
SPIFFE authority, docs, the reference retention cleanup timer, and CLI packages.
...
database with the matching DSN. Do not start workers, cron, SCM trigger
producers, reconciler, catalog, or API against a database whose schema status is
unknown.
...
- scrape retained evidence metrics from the `vectis-cli retention evidence metrics` output, such as `/var/lib/vectis/ops/retention-evidence.prom` in the reference timer;
