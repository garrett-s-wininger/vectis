...
| `vectis-scm-poller` | 1+ | Yes, within one DB cell | SCM poll trigger claims select one poll attempt for a due trigger row. Each discovered provider event is recorded under a stable event key so duplicate polls do not duplicate trigger events. Run multiple replicas against the same global database for failover; use `--instance-id` / `VECTIS_SCM_POLLER_INSTANCE_ID` to make claim ownership readable. |
...
SCM poller scale-out follows the same claim pattern for `scm_poll_trigger_specs`. Replicas race on due poll rows, only the winner contacts the provider for that trigger, and discovered changes are deduplicated through `scm_trigger_events` by stable event key.
...
| `vectis-scm-poller` | SCM trigger scans pause if all poller instances are down. A crash after recording a provider event but before later dispatch work can be retried from durable state once run creation is wired to the event ledger. | Run one or more poller instances against the same global database. Gate them on database reachability and provider network policy. |
