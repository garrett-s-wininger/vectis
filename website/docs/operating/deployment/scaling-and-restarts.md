...
| `vectis-scm-poller` | 1+ | Yes, within one DB cell | SCM poll trigger claims select one poll attempt for a due trigger row. Each discovered provider event is recorded under a stable event key and linked to one dispatched run, so duplicate polls do not duplicate work. Run multiple replicas against the same global database for failover; use `--instance-id` / `VECTIS_SCM_POLLER_INSTANCE_ID` to make claim ownership readable. |
| `vectis-scm-gerrit-stream` | 0+ | Yes, by event-key dedupe | Optional Gerrit stream-events bridge. Managed SSH mode owns host-key verification, key/agent auth, the `gerrit stream-events` command, and reconnect backoff, but it does not claim exclusive stream ownership in Vectis; run it where the external Gerrit stream subscription topology is deliberate. Duplicate stream consumers can observe the same event, but the shared SCM trigger event ledger links each normalized event key to one dispatched run. Use `--instance-id` / `VECTIS_SCM_GERRIT_STREAM_INSTANCE_ID` to make the bridge process visible in trigger invocation and dispatch audit rows. |
...
Each artifact shard has a stable identity and serves the blobs routed to that shard. With the local backend, artifact scale-out comes from adding shards, not from starting multiple active processes on the same artifact storage path. `vectis-artifact` takes an advisory lock on the local storage directory and refuses to start when another process already owns it. If a local shard falls below `--storage-read-only-min-free-bytes`, it advertises read-only for new blobs and rejects uploads for blobs it does not already store while continuing to serve existing blobs. With the S3 backend, object durability and capacity are owned by the configured S3-compatible service; Vectis still records which artifact shard handled the upload.
...
| `vectis-artifact` | Uploads and reads routed to that shard are interrupted. Durable blobs remain if local storage is preserved or the S3-compatible backend remains durable. | Keep each shard on a stable `--instance-id`. For local storage, use a durable, separate `--storage-dir`; when omitted, defaults are derived from `hostname-port`. For S3 storage, preserve bucket/prefix and credentials. |
...
| `vectis-scm-poller` | SCM trigger scans pause if all poller instances are down. A crash after recording a provider event but before creating its run can be retried from the event ledger; a crash after run creation is handled by the queued-run reconciler. | Run one or more poller instances against the same global database. Gate them on database reachability and provider network policy. |
| `vectis-scm-gerrit-stream` | Gerrit stream-events stop being observed if all bridge instances or their upstream SSH streams are down. Already recorded events remain retryable through the event ledger and queued-run reconciler. | Run only the external stream consumers your Gerrit topology expects. Gate them on database, queue, and Gerrit stream reachability. |
...
| Artifacts | Run-scoped artifact list, metadata, and download APIs backed by worker-originated uploads to content-addressed artifact storage. Local storage shards use exclusive directories and disk-pressure read-only mode; S3-compatible storage is available through `vectis-artifact --storage-backend=s3`. There is no automatic artifact replication, automatic read failover to a different shard, or public API upload path. |
