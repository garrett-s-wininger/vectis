...
| SCM trigger producers | Run `vectis-scm-poller` only if SCM polling triggers are used, and `vectis-scm-gerrit-stream` only if Gerrit stream-events are used. Poller instances coordinate through database trigger claims; stream bridges rely on shared event-key dedupe. |
