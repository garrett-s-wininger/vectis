...
| Expose local API, UI, and docs from a dev host | `VECTIS_DOCS_ALLOWED_HOSTS=<dev-host> vectis-local --host 0.0.0.0` |
...
| Require cleanup evidence by policy | `VECTIS_RETENTION_CLEANUP_REQUIRE_BACKUP_MANIFEST=true`, `VECTIS_RETENTION_CLEANUP_BACKUP_MAX_AGE=24h`, `VECTIS_RETENTION_CLEANUP_REQUIRE_AUDIT_EXPORT=true`, `VECTIS_RETENTION_CLEANUP_AUDIT_EXPORT_MAX_AGE=24h`, `VECTIS_RETENTION_CLEANUP_REQUIRE_HOLD_REVIEW=true`, and `VECTIS_RETENTION_CLEANUP_HOLD_REVIEW_MAX_AGE=24h` |
| Use a scheduled cleanup evidence manifest | `VECTIS_RETENTION_CLEANUP_EVIDENCE_MANIFEST=/var/lib/vectis/ops/retention-cleanup-evidence.json` |
...
Each entry accepts `repository_id`, `namespace`, `source_kind`, `checkout_path`, `checkout_mode`, `authoring_mode`, `worker_cache_mode`, `canonical_url`, `fallback_remote_urls`, `worker_cache_warm_refspecs`, `default_ref`, `credential_ref`, and `enabled`. `namespace` defaults to `/`, `source_kind` defaults to `local_checkout`, `checkout_mode` defaults to `external`, `authoring_mode` defaults to `read_only`, `worker_cache_mode` defaults to `ephemeral`, and `enabled` defaults to `true`. Set `worker_cache_mode` to `persistent` for large or high-churn repositories workers should keep warm; leave it `ephemeral` for smaller repositories that can use short-lived worker-side materialization. `worker_cache_warm_refspecs` narrows persistent worker-cache warming to explicit Git fetch refspecs such as `+refs/heads/*:refs/heads/*`, `+refs/tags/*:refs/tags/*`, `+refs/notes/*:refs/notes/*`, `+refs/pull/*:refs/pull/*`, or `+refs/changes/*:refs/changes/*`; when omitted, worker-core keeps the previous broad `+refs/*:refs/*` mirror behavior. For `checkout_mode=managed`, omit `checkout_path`; Vectis derives a stable path under `source.checkout_root` / `VECTIS_SOURCE_CHECKOUT_ROOT`. `fallback_remote_urls` configures higher replica tiers or the upstream source provider for request-time managed ref hydration when `canonical_url`/`origin` has not caught up yet.

Managed checkout sync and persistent worker cache warming can use `credential_ref` for private repositories. `vectis-api` resolves the reference for managed source checkout sync, `vectis-worker` resolves it before sending credential material over the local worker-core socket for DB-driven persistent checkout cache warms, and standalone `vectis-worker-core` resolves it for static persistent cache declarations. With the current encryptedfs provider, configure `secrets.providers.encryptedfs.root` / `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_ROOT` and `secrets.providers.encryptedfs.key_file` / `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_KEY_FILE`, or use `--encryptedfs-root` and `--encryptedfs-key-file` on the services that need source repository credentials. The referenced secret may contain either a raw HTTPS token, JSON like `{"username":"oauth2","token":"..."}` or `{"username":"git","password":"..."}`, raw SSH private key material, or JSON like `{"ssh_private_key":"...","known_hosts":"git.example ssh-ed25519 ..."}`. HTTPS credentials are supplied to `git clone` and `git fetch` through askpass environment variables, while SSH credentials are supplied through a temporary identity file and `GIT_SSH_COMMAND`. Include `known_hosts` to pin SSH host keys; when it is omitted, OpenSSH's normal host-key verification applies. Credentials are not embedded in the remote URL.

Startup reconciliation creates missing repository registrations and updates changed checkout, authoring, worker cache, fallback remote, warm refspec, default ref, credential, and enabled fields. It does not delete repositories omitted from config. If a configured repository already exists in a different namespace, startup fails so operators can make an explicit move plan.
...
| --- | --- | --- |
| `vectis-api` | `VECTIS_API_SERVER` | `--host`, `--port`, `--cell-ingress-endpoint`, `--tls-cert-file`, `--tls-key-file`, `--encryptedfs-root`, `--encryptedfs-key-file` |
...
| `vectis-secrets` | `VECTIS_SECRETS` | `--port`, `--metrics-host`, `--metrics-port`, `--encryptedfs-root`, `--encryptedfs-key-file`, `--knox-url`, `--knox-auth-token-file`, `--knox-auth-token`, `--knox-insecure-skip-verify`, `--knox-ca-file`, `--knox-client-cert-file`, `--knox-client-key-file`, `--allow-secret` |
...
| `VECTIS_API_AUTH_LDAP_SUBJECT_ATTRIBUTE` / `api.auth.ldap.subject_attribute` | LDAP attribute used as the stable external subject, such as `entryUUID` for OpenLDAP or `objectGUID` for Active Directory. Defaults to the entry DN when empty. |
...
| `VECTIS_API_AUTH_LDAP_AUTO_LINK_USERS` / `api.auth.ldap.auto_link_users` | When `true`, a first-seen LDAP subject may be linked to an existing local user with the same mapped username. Defaults to `true`; set `false` to require explicit pre-linking. |
...
Use `api.auth.ldap.subject_attribute` for a stable directory subject; leaving it
empty uses the entry DN. That local user ID is used for sessions, API tokens,
role bindings, and audit actor IDs. If no link exists yet, the LDAP username
attribute is used to find or auto-create the local user according to
`api.auth.ldap.auto_link_users` and `api.auth.ldap.auto_create_users`.
Operators can pre-link an accepted provider subject with
`vectis-cli users external-identities link` before the user's first login.
On a fresh database, setup can bind the first admin to an already registered
provider subject in the same bootstrap transaction:
...
```json
{
  ...
  "external_identity": {
    ...
    "subject": "entryUUID-or-objectGUID",
    "username": "alice"
  }
}
```

When `password_auth_enabled=false`, the first admin has no usable local password
login path; the setup response still returns the initial API token for recovery
and automation. Admin-set password changes explicitly re-enable local password
auth for a user.
...
`vectis-secrets` routes secret refs by URI scheme. `encryptedfs://...` resolves encrypted envelope files below `--encryptedfs-root`. `knox://...` resolves the primary version of a key from a configured Knox service. Knox key IDs use Knox's `service:key` shape; Vectis accepts `knox://service/key`, `knox://service:key`, `knox:///service:key`, and `knox:service:key` forms. Provider-owned config keys live under `secrets.providers.<provider>.*`; the matching environment names are `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_ROOT`, `VECTIS_SECRETS_PROVIDERS_ENCRYPTEDFS_KEY_FILE`, `VECTIS_SECRETS_PROVIDERS_KNOX_URL`, `VECTIS_SECRETS_PROVIDERS_KNOX_AUTH_TOKEN_FILE`, `VECTIS_SECRETS_PROVIDERS_KNOX_CA_FILE`, `VECTIS_SECRETS_PROVIDERS_KNOX_CLIENT_CERT_FILE`, and `VECTIS_SECRETS_PROVIDERS_KNOX_CLIENT_KEY_FILE`. Enable Knox with `--knox-url` plus either `--knox-auth-token-file` or `--knox-auth-token`; prefer the token file so the Authorization header value is not visible in process arguments. Configure `--knox-ca-file` for private Knox server trust roots, and configure both `--knox-client-cert-file` and `--knox-client-key-file` when the Knox endpoint requires mTLS.
...
Workers keep dequeue connections low by sampling one queue shard at a time. `VECTIS_WORKER_QUEUE_DEQUEUE_STICKY_SUCCESS_BUDGET` / `--queue-dequeue-sticky-success-budget` controls how many successful dequeues a worker may take from a productive shard before probing another shard; it defaults to `64`. Lower values spread workers across new or newly-busy shards faster, while higher values reduce shard switching and connection churn. Idle workers use bounded long-poll dequeue RPCs with exponential backoff after empty long-polls. `VECTIS_WORKER_QUEUE_DEQUEUE_POLL_BASE_INTERVAL` / `--queue-dequeue-poll-base-interval` defaults to `250ms`, `VECTIS_WORKER_QUEUE_DEQUEUE_POLL_MAX_INTERVAL` / `--queue-dequeue-poll-max-interval` defaults to `1s`, and `VECTIS_WORKER_QUEUE_DEQUEUE_POLL_JITTER_RATIO` / `--queue-dequeue-poll-jitter-ratio` defaults to `0.2` for one-sided 80-100% jitter. Increasing the max interval lowers steady-state empty dequeue RPC completions at the cost of slower cross-shard pickup when new work arrives to a shard a worker is not currently long-polling; set jitter ratio to `0` to disable jitter. `VECTIS_WORKER_QUEUE_CONTINUATION_INLINE_JOB_MAX_BYTES` / `--queue-continuation-inline-job-max-bytes` defaults to `65536`; continuation deliveries inline the full job when the serialized job is at or below that size, otherwise workers enqueue a compact task delivery backed by the durable execution payload hash. Set it to `0` to always use compact continuation deliveries.
...
Set `VECTIS_WORKER_CORE_CHECKOUT_CACHE_ROOT` or `--checkout-cache-root` to enable worker-local persistent checkout mirrors for `builtins/checkout`. Worker-core caches only source repositories whose `worker_cache_mode` is `persistent`; provide static worker-core declarations through `source.repositories`, `VECTIS_SOURCE_REPOSITORIES`, `VECTIS_WORKER_SOURCE_REPOSITORIES`, or `VECTIS_WORKER_CORE_SOURCE_REPOSITORIES`. The worker also reads enabled persistent source repository rows from the cell database and asks worker-core to warm those mirrors before task checkout uses them. Persistent worker-cache warm requests preserve each repository's `fallback_remote_urls` as ordered tiers for the canonical mirror instead of treating them only as unrelated remotes, honor `worker_cache_warm_refspecs`, and include resolved `credential_ref` material when the worker is configured with the encryptedfs source credential provider. Static worker-core persistent declarations with `credential_ref` require the same encryptedfs source credential provider on `vectis-worker-core`. Tune `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_WARM_INTERVAL`, `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_WARM_TIMEOUT`, `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_WARM_JITTER_RATIO`, `VECTIS_WORKER_CORE_CHECKOUT_CACHE_WARM_PARALLELISM`, and `VECTIS_WORKER_CORE_CHECKOUT_CACHE_MAX_BYTES` for very large repositories or large worker fleets. Repositories left at the default `ephemeral` mode keep using direct per-workspace clones.

Worker-core installs cache refreshes as immutable mirror generations and atomically flips a `current` pointer after a generation is complete. Task checkouts clone from the current complete generation without waiting for an in-flight refresh when one already exists; an empty cache still warms once on demand before checkout can use it. During mirror refresh, worker-core fetches the canonical remote first and can fetch configured fallback tiers into the same canonical mirror so just-landed provider refs or notes available upstream can reach the worker cache before lower-tier mirrors catch up. When a current generation already exists, worker-core first compares advertised canonical-plus-fallback refs to the current mirror and skips building a receiving generation if they match; if the full refresh still produces the same ref snapshot, worker-core discards that receiving generation and leaves `current` untouched to avoid cache churn. Cached workspaces hardlink local objects when the cache and workspace filesystem allow it. When hardlinks are unavailable, such as across filesystems, task-scoped checkouts borrow objects through Git alternates while holding a generation lease until the task finishes; non-scoped cache users still copy objects so their workspaces do not depend on cache lifetime. For best large-repository checkout latency and disk efficiency, place `worker.execution.checkout_cache_root` and `worker.execution.workspace_root` on the same filesystem, but arbitrary filesystem layouts remain supported. Each cached workspace keeps `origin` pointed at the declared remote and adds a `vectis-cache` remote pointing at the worker-local `current` mirror for explicit local fetches of extra refs such as `refs/notes/*`, GitHub pull refs, or Gerrit change refs. If `fetch_refspecs` asks for refs outside the warm policy, worker-core hydrates those source refs into a new cache generation before the workspace fetches them locally. After each changed refresh, worker-core keeps `worker.execution.checkout_cache_generations_to_keep` generations per remote, defaulting to two, and removes older unleased generations; when `worker.execution.checkout_cache_max_bytes` is positive, cleanup also removes oldest unleased, non-current generations until retained pack bytes are under the per-remote budget or only protected generations remain. Active checkout leases refresh while the checkout is in flight and keep an older generation in place until the checkout releases it. If a cached clone or auxiliary ref fetch fails with a missing-object/corruption-style error, worker-core retires the current generation only if it is still current, rewarms, and retries once. If worker-core exits before releasing a checkout lease, later cleanup treats the lease as stale after `worker.execution.checkout_cache_lease_ttl`, defaulting to one hour. The same TTL bounds abandoned `.receiving` generation directories left by an interrupted cache refresh; worker-core prunes stale receive directories before starting a later refresh.

Worker-core exposes its own metrics listener with `VECTIS_WORKER_CORE_METRICS_HOST` / `VECTIS_WORKER_CORE_METRICS_PORT`. Scrape it alongside `vectis-worker`: worker-core metrics expose checkout cache hits, misses, cache errors, clone modes, direct clone fallbacks, and checkout action latency, while worker metrics expose warm passes with changed versus unchanged remotes.
...
| `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_MAX_BYTES` / `VECTIS_WORKER_CORE_CHECKOUT_CACHE_MAX_BYTES` | Per-remote checkout cache pack-byte budget. Defaults to `0`, disabled. |
...
| `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_WARM_JITTER_RATIO` | Stable per-worker jitter ratio applied to cache warm scheduling. Defaults to `0.2`. |
| `VECTIS_WORKER_EXECUTION_CHECKOUT_CACHE_WARM_PARALLELISM` / `VECTIS_WORKER_CORE_CHECKOUT_CACHE_WARM_PARALLELISM` | Worker-core remotes to warm concurrently during one checkout cache warm pass. Defaults to `1`. |
