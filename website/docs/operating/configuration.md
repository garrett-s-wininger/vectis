...
| `vectis-secrets` | `VECTIS_SECRETS` | `--port`, `--metrics-host`, `--metrics-port`, `--encryptedfs-root`, `--encryptedfs-key-file`, `--knox-url`, `--knox-auth-token-file`, `--knox-auth-token`, `--knox-insecure-skip-verify`, `--knox-client-cert-file`, `--knox-client-key-file`, `--allow-secret` |
...
role bindings. For local compatibility validation, run `make ldap-smoke-up` and
`make ldap-smoke-check` to exercise service-account search, user bind, identity
mapping, and wrong-password rejection against OpenLDAP.
