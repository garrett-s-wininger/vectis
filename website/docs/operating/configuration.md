...
| Variable / key | Purpose |
| --- | --- |
| `VECTIS_WORKER_SPIRE_ENABLED` / `worker.spire.enabled` | Enables worker-side SPIRE Workload API integration and makes every execution acquire a matching X.509-SVID before action code runs. Requires `worker.execution_identity.enabled=true` and an explicit Workload API address. |
...
| `VECTIS_WORKER_SPIRE_REGISTRATION_ENABLED` / `worker.spire.registration.enabled` | Enables worker-controlled SPIRE Server Entry API registration before the worker fetches the execution SVID. Requires `worker.spire.enabled=true`. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_SERVER_ADDRESS` / `worker.spire.registration.server_address` | SPIRE Server API Unix socket address, such as `unix:///run/spire/sockets/server.sock`. The first implementation only supports local `unix:///...` addresses. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_PARENT_ID` / `worker.spire.registration.parent_id` | Parent SPIFFE ID for execution registration entries, usually the attested worker node or agent identity. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_SELECTORS` / `worker.spire.registration.selectors` | Trusted workload selectors in `type:value` form, for example `unix:uid:1000` or `k8s:sa:vectis:worker`. Values may contain additional `:` characters. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_X509_SVID_TTL` / `worker.spire.registration.x509_svid_ttl` | Optional X.509-SVID TTL to set on created SPIRE entries. Defaults to `0s`, which leaves the SPIRE server default in effect. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_MIN_TTL` / `worker.spire.registration.min_ttl` | Optional minimum accepted lifetime for generated registration intents. Defaults to `0s`. |
| `VECTIS_WORKER_SPIRE_REGISTRATION_MAX_TTL` / `worker.spire.registration.max_ttl` | Optional maximum accepted lifetime for generated registration intents. Defaults to `0s`. |
...
6. The built-in registrar uses the SPIRE Server Entry API through a local Unix socket. Protect that socket as registration authority and expose it only to the Vectis worker process or a dedicated registrar component.
7. The registration workflow should live outside arbitrary job actions. Do not give shell steps the SPIRE Workload API socket, the SPIRE Server API socket, the SVID private key, or registration authority credentials; secret brokering consumes the worker-owned acquired SVID instead.
