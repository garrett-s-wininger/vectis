...
Docs, cell ingress, and dedicated metrics listeners also reject unsafe request targets before handler logic runs, including absolute-form proxy request targets, `OPTIONS *`, percent-encoded path text, duplicate slash paths, and dot segments. Cell ingress route matching is similarly narrow: health endpoints allow `GET`/`HEAD`, execution submission allows `POST`, and other paths or methods return JSON errors before handler logic runs. Cell ingress also enforces JSON-compatible `Accept` values. Cell ingress responses use the shared baseline security headers and `Cache-Control: no-store`.
