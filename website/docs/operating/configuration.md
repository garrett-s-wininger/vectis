...
| `VECTIS_WORKER_SPIRE_REQUIRE_EXECUTION_SVID` / `worker.spire.require_execution_svid` | Explicit fail-closed assertion for deployments that require execution SVID proof. `worker.spire.enabled=true` already enforces this gate. |
