...
| --- | --- | --- | --- |
| Global control plane | `vectis-api`, `vectis-cron`, SCM trigger producers such as `vectis-scm-poller` and `vectis-scm-gerrit-stream`, `vectis-reconciler`, `vectis-catalog` | Global DB | Store jobs and global run catalog, accept user/API/SCM triggers, repair missed dispatch, fan in cell events. |
...
Start `vectis-api`, `vectis-cron`, the SCM trigger producers you use, `vectis-reconciler`, and `vectis-catalog` against the global database. Start `vectis-cell-ingress`, `vectis-queue`, `vectis-worker`, and `vectis-worker-core` in each cell against that cell's database and queue. If a deployment keeps the default `local` cell, give it the same treatment as every other cell once global and cell databases are split.
