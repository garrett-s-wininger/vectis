...
This page is for **people running or integrating Vectis** (operators, platform engineers, and developers wiring staging or prod). It focuses on **what to set**—environment variables, ports, and discovery—not on Go package layout. For service roles and data flow, see [ARCHITECTURE.md](../developer/architecture.md). For **Prometheus `/metrics`** ports and env on each binary, use the **Default ports** and **Common operator settings** sections below; roadmap context is in [PLANNING.md](../developer/planning.md) §10. For **job**, **run**, **enqueue**, and related terms, see [GLOSSARY.md](../general/glossary.md). For **secrets and trust boundaries**, see [SECURITY.md](../general/security.md).
...
| **`VECTIS_API_AUTHZ_ENGINE`** / **`api.authz.engine`** | `hierarchical_rbac` (default) or `authenticated_full`. See [SECURITY.md](../general/security.md). |
...
| Variable | Purpose |
| --- | --- |
| **`VECTIS_GRPC_TLS_INSECURE`** | If `true`, use plaintext gRPC (default for **standalone** binaries). If `false`, PEM paths below must satisfy the role of each binary (see [SECURITY.md](../general/security.md)). **`vectis-local` normally sets this to `false` via bootstrap** unless `--grpc-insecure` / **`VECTIS_LOCAL_GRPC_INSECURE`**. |
...
Viper keys `database.pgx_pool.*` apply where a binary loads full config into viper (same semantics as other `database.*` keys). See [FAILURE_DOMAINS.md](../general/failure-domains.md#database) for why this matters under outages and many processes.
...
For startup ordering and readiness meaning across binaries, see [FAILURE_DOMAINS.md](../general/failure-domains.md#startup-and-recovery-matrix). In short: pin queue/log addresses when you want to remove registry from the dependent process startup path, and use readiness to gate new work rather than to decide whether a supervisor should restart a process.

Retry defaults and retry-exhaustion guidance are listed in [RETRY_POLICY.md](../developer/retry-policy.md). Most attempt counts are code defaults today; the operator-facing knobs are discovery timing, reconciler interval, queue persistence, log-forwarder spool durability, and database pool settings.
...
| Enable HTTP API auth (Bearer after setup) | `VECTIS_API_AUTH_ENABLED=true` and bootstrap token for new DBs — see [HTTP API authentication](#http-api-authentication-vectis-api) and [SECURITY.md](../general/security.md) |
...
Treat these paths, deployment secrets, and TLS material as part of the backup set when they hold production data. See [BACKUP_RESTORE.md](backup-restore.md) for restore order and partial-restore outcomes.

## Related documentation

| Topic | Document |
| --- | --- |
| Components and flows | [ARCHITECTURE.md](../developer/architecture.md) |
| Failure behavior | [FAILURE_DOMAINS.md](../general/failure-domains.md) |
| Runbooks, SLOs, and alert examples | [RUNBOOKS.md](runbooks.md) |
| Repair recipes | [REPAIR_RUNBOOKS.md](repair-runbooks.md) |
| Startup dependencies and probes | [FAILURE_DOMAINS.md](../general/failure-domains.md#startup-and-recovery-matrix) |
| Dispatch handoff triage | [DISPATCH_VISIBILITY.md](dispatch-visibility.md) |
| Log streaming and replay behavior | [LOG_STREAMING.md](../user/log-streaming.md) |
| Backup, restore, and disaster recovery | [BACKUP_RESTORE.md](backup-restore.md) |
| Context and deadline policy | [CONTEXTS.md](../developer/contexts.md) |
| Retry and backoff defaults | [RETRY_POLICY.md](../developer/retry-policy.md) |
| Roadmap / persistence | [PLANNING.md](../developer/planning.md) §2.5 |
| Quick start, Postgres / Podman | [README.md](https://github.com/garrett-s-wininger/vectis#readme) |
| Glossary | [GLOSSARY.md](../general/glossary.md) |
| Security posture | [SECURITY.md](../general/security.md) |
| Reference deployment posture | [REFERENCE_DEPLOYMENT_POSTURE.md](reference-deployment-posture.md) |
| Releases and upgrades | [RELEASES.md](../developer/releases.md) |
