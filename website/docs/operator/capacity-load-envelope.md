...
The envelope above is a throughput and pressure statement. The scale-out contract in [SCALING_AND_RESTARTS.md](scaling-and-restarts.md) remains the source of truth for whether a component is safe to run as multiple active replicas.
...
- Log replay overload is bounded by the HTTP replay controls in [LOG_STREAMING.md](../user/log-streaming.md).
