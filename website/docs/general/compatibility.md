...
This page defines the compatibility contract for Vectis clients, operators, and alternate component implementations. It complements [API_REFERENCE.md](../user/api-reference.md), [MIGRATIONS.md](../developer/migrations.md), and [RELEASES.md](../developer/releases.md).
...
- Error responses use the envelopes documented in [API_REFERENCE.md](../user/api-reference.md). New `code` values may be added for more specific failures; existing code meanings should remain stable.
...
- Service env prefixes are part of the contract for each binary; update [CONFIGURATION.md](../operator/configuration.md) and `cmd/AGENTS.md` together when they change.

## Schema And Releases

Database compatibility follows [MIGRATIONS.md](../developer/migrations.md) and [ADR 0004](../developer/adr/0004-migration-compatibility-and-rollback.md). Release notes must say whether old binaries can run against the new schema, whether new binaries can start before migration, and whether mixed binary versions are supported.

Default deployment guidance remains: run one Vectis release version across long-running services unless [RELEASES.md](../developer/releases.md) or release notes explicitly allow version skew.
