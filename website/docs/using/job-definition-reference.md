...
| `builtins/checkout` | Required `url`; optional `fetch_refspecs` | `url`, `fetch_refspecs` | None | No | Runs `git clone <url> .` with terminal prompts disabled. HTTP(S) URLs with embedded credentials are rejected; SCP-style Git URLs are accepted. When worker persistent cache handles the URL, `origin` stays on the declared remote and `vectis-cache` exposes the local mirror; `fetch_refspecs` fetches whitespace-separated refspecs from a refreshed cache mirror on cache hits or `origin` after direct clones. |
