...
| `builtins/upload-artifact` | Required `name`, `path`; optional `content_type`, `metadata_json`, `max_bytes` | None | None | No | Publishes a workspace-relative file as a run artifact and returns an `artifact` object with blob and size metadata. |
