...
Persistent worker cache warming also uses `credential_ref` when `vectis-worker` is configured with the encryptedfs source credential provider. The worker resolves the reference before asking worker-core to warm the mirror, so private persistent repositories can stay warm without embedding credentials in clone URLs.
