...
Set `source.stored_jobs_enabled` to `false` (or `VECTIS_SOURCE_STORED_JOBS_ENABLED=false`) for source-only deployments that should not create, update, read, trigger, or stream stored jobs. Stored-job routes then return `409 stored_jobs_disabled`; one-off runs and direct source repository triggers remain available.
