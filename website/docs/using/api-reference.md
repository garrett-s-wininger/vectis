...
curl -sS \
  -X POST \
  -H 'Content-Type: application/json' \
  -d '{"repository_id":"vectis-local","job_id":"build","branch":"main","message":"Add build job","job":{"root":{"id":"root","uses":"builtins/script","with":{"script":"mage test"}}}}' \
  http://localhost:8080/api/v1/jobs

curl -sS \
  -X PUT \
  -H 'Content-Type: application/json' \
  -d '{"repository_id":"vectis-local","branch":"main","message":"Update build job","job":{"root":{"id":"root","uses":"builtins/script","with":{"script":"mage testQuick"}}}}' \
...
| GET | `/api/v1/audit/events` | List audit events with optional event, actor, target, correlation, time, cursor, and limit filters | `admin:*` | none | `200` JSON audit event list |
...
| POST | `/api/v1/cells/{cell_id}/catalog-events` | Record a cell status event into the global catalog inbox | `catalog:ingest` | general | `202` JSON event |
