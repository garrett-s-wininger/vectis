...
Workers can derive an expected per-execution SPIFFE ID from the execution envelope with `worker.execution_identity.*`. That identity includes cell, namespace, job, run, and execution scope and is attached only to Vectis' in-process action state and tracing. If `worker.spire.enabled=true`, the worker requires its SPIRE Workload API source to return an X.509-SVID with that exact SPIFFE ID before action code runs. When a task declares secrets, the worker uses that SVID as the gRPC client certificate for `vectis-secrets`; the broker independently derives the expected execution identity from the active execution record, requires the mTLS peer SPIFFE ID to match, and then applies operator-owned secret access policy before returning material. Vectis does not expose a SPIRE Workload API socket, SVID, private key, or derived SPIFFE ID to shell commands.
