...
| Internal gRPC | Queue, registry, orchestrator, log, artifact, secrets, and worker-control traffic can use optional TLS/mTLS. Standalone binaries default to plaintext unless configured. | Keep internal ports private. Use TLS/mTLS and service identity allowlists where available on shared networks. |
...
| Service authorization | Internal gRPC listeners and cell-ingress execution submission can enforce role-level exact SPIFFE URI SAN allowlists when configured for implemented roles. This is listener authorization, not a full per-RPC policy language. | Configure expected service identities and still treat network reachability to queue, registry, orchestrator, log, artifact, secrets, cell ingress, and worker-control paths as sensitive. |
...
| Database and files | SQL, SQLite files, queue persistence, logs, artifact blobs, job secret stores, and backups may contain sensitive operational state. Vectis does not encrypt these at rest itself. | Use platform disk or volume encryption, filesystem permissions, and secret-store controls. |
...
| `vectis-secrets` | Resolves declared job secrets for authenticated, claimed executions. |
| API, worker, cell ingress, cron, reconciler | Dial queue, registry, orchestrator, log, artifact, or secrets depending on role and configuration. |
...
Workers run actions from job definitions. A script action or checkout source should be treated as code execution capability.
...
Process-launching built-ins such as `builtins/script`, `builtins/test`, and `builtins/checkout` receive a minimal Vectis-built environment instead of inheriting the worker service environment. The child environment includes stable process basics such as `PATH`, workspace-local `HOME`/`TMPDIR`, and `CI=true`; it strips deployment variables such as database DSNs, TLS settings, API bootstrap material, SPIFFE endpoint sockets, dynamic-loader hooks, SSH agent sockets, cloud credential hints, kubeconfigs, and common package or source-host tokens. Command-name resolution uses the child environment's `PATH`, falls back to Vectis' default system path, and ignores empty or relative path entries for the initial executable lookup. On Unix hosts, Vectis also starts each launched command in its own process group, so process-group signals from action code, such as `kill 0`, stay out of the worker process group. Cancellation and worker-core shutdown signal command process groups before force-killing them, and Linux launch leaders receive a parent-death signal to reduce orphaned action processes if the worker core exits unexpectedly. Launched commands must have an explicit non-symlink working directory, which Vectis canonicalizes before launch; they read from the null device unless an action explicitly supplies stdin, and receive no extra file descriptors unless the process launcher explicitly configures them. Unix host execution routes through a short-lived authenticated launcher process that resets selected inherited signal dispositions, resets the final exec thread's signal mask on supported hosts, and applies an owner-only `077` umask before replacing itself with the action command. Custom process action `working_directory` values must be relative and stay under the action base directory. Jobs can still read any files or mounts that the worker runtime exposes to them, so worker isolation remains the stronger boundary.

Auto-created workspaces are direct `vectis-*` children of the configured workspace root and are forced to owner-only permissions before action code runs. Workspace cleanup validates canonical root and workspace paths, rejects terminal workspace symlinks, and refuses paths that are empty, equal to the root, outside the root, nested below the direct workspace child, or missing the `vectis-` prefix. Workspace-local runtime directories such as `.tmp` and `.vectis` must be real directories, not symlinks, and are forced to owner-only permissions on Unix hosts. Explicit workspaces must be real directories, are preserved after the run, and emit a warning when group or world accessible.

Materialized secret files are written only under the workspace secrets directory, reject symlinked path components, and enforce owner-only non-executable modes. Artifact uploads, script output files, and local action manifests resolve symlinks before the worker reads them and must remain inside their configured workspace or action root. Durable local log spool directories are owner-only and reject symlinked directories and spool files. Worker-core and shell callback Unix-domain sockets are created with owner-only socket permissions inside real owner-only runtime directories, and supported Unix platforms verify that local socket peers run as the same UID. These controls protect the local worker control plane from other users on the same host; they are not a substitute for workload isolation.

Workers can derive an expected per-execution SPIFFE ID from the execution envelope with `worker.execution_identity.*`. That identity includes cell, namespace, job, run, and execution scope and is attached only to Vectis' in-process action state and tracing. If `worker.spiffe.enabled=true`, the worker requires its SPIFFE Workload API source to return an X.509-SVID with that exact SPIFFE ID before action code runs. When a task declares secrets, the worker uses that SVID as the gRPC client certificate for `vectis-secrets`; the broker independently derives the expected execution identity from the active execution record, requires the mTLS peer SPIFFE ID to match, and then applies operator-owned secret access policy before returning material. Vectis does not expose a SPIFFE Workload API socket, SVID, private key, or derived SPIFFE ID to job subprocesses.
