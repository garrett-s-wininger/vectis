...
Auto-created workspaces are direct `vectis-*` children of the configured workspace root and are forced to owner-only permissions before action code runs. Cleanup refuses paths that are empty, equal to the root, outside the root, nested below the direct workspace child, or missing the `vectis-` prefix. Explicit workspaces are preserved after the run and emit a warning when group or world accessible.

Materialized secret files are written only under the workspace secrets directory, reject symlinked path components, and enforce owner-only non-executable modes. Worker-core and shell callback Unix-domain sockets are created with owner-only socket permissions inside owner-only runtime directories, and supported Unix platforms verify that local socket peers run as the same UID. These controls protect the local worker control plane from other users on the same host; they are not a substitute for workload isolation.
