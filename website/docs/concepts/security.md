...
Process-launching built-ins such as `builtins/shell` and `builtins/checkout` receive a minimal Vectis-built environment instead of inheriting the worker service environment. The child environment includes stable process basics such as `PATH`, workspace-local `HOME`/`TMPDIR`, and `CI=true`; it strips deployment variables such as database DSNs, TLS settings, API bootstrap material, and SPIFFE endpoint sockets. Jobs can still read any files or mounts that the worker runtime exposes to them, so worker isolation remains the stronger boundary.
