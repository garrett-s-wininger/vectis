...
Process-launching built-ins such as `builtins/shell` and `builtins/checkout` receive a minimal Vectis-built environment instead of inheriting the worker service environment. The child environment includes stable process basics such as `PATH`, workspace-local `HOME`/`TMPDIR`, and `CI=true`; it strips deployment variables such as database DSNs, TLS settings, API bootstrap material, and SPIFFE endpoint sockets. On Unix hosts, Vectis also starts each launched command in its own process group, so process-group signals from action code, such as `kill 0`, stay out of the worker process group. Cancellation signals the command process group before force-killing it, and Linux launch leaders receive a parent-death signal to reduce orphaned action processes if the worker core exits unexpectedly. Launched commands read from the null device unless an action explicitly supplies stdin, and they receive no extra file descriptors unless the process launcher explicitly configures them. Jobs can still read any files or mounts that the worker runtime exposes to them, so worker isolation remains the stronger boundary.

Worker-core and shell callback Unix-domain sockets are created with owner-only socket permissions inside owner-only runtime directories. This protects the local worker control plane from other users on the same host; it is not a substitute for workload isolation.
